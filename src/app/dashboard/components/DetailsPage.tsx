...
import { formatDate, timeZoneMap } from "@/lib/Timezone";
...
export default function DetailsPage({ user }: { user: User }) {
  ...
  const [loadingTimeInStatus, setLoadingTimeInStatus] = useState<boolean>(true);
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
                ...
                {user.attendances.map((attendance) => (
                  <tr key={attendance.id}>
                    <td className="px-4 py-2 whitespace-nowrap">
                      {attendance.timeIn
                        ? formatDate(
                            new Date(attendance.timeIn).toISOString(),
                            timeZoneMap[selectedTimeZone]
                          )
                        : "N/A"}
                    </td>
                    <td className="px-4 py-2 whitespace-nowrap">
                      {attendance.timeOut
                        ? formatDate(
                            new Date(attendance.timeOut).toISOString(),
                            timeZoneMap[selectedTimeZone]
                          )
                        : "N/A"}
                    </td>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        <h2 className="text-2xl font-semibold mb-4">Your Overtime Request</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-100">
              <tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {user.overtimeRequests.map((otr) => (
                <tr key={otr.id}>
                  <td className="px-4 py-2 whitespace-nowrap">{otr.hours}</td>
                  <td className="px-4 py-2 whitespace-nowrap">{otr.type}</td>
                  <td className="px-4 py-2 whitespace-nowrap">{otr.status}</td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    {formatDate(
                      new Date(otr.createdAt).toISOString(),
                      timeZoneMap[selectedTimeZone]
                    )}
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    {formatDate(
                      new Date(otr.updatedAt).toISOString(),
                      timeZoneMap[selectedTimeZone]
                    )}
                  </td>
                </tr>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="flex gap-4 justify-center">
          <TimeInButton userId={user.id} hasTimeInToday={hasTimeInToday} />
          <TimeOutButton
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal}>
        <OvertimeRequestForm userId={user.id} />
      </Modal>
    </div>
  );
}
